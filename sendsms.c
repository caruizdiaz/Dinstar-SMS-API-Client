...
int main(int argc, char** argv)
{
	if (argc != 5)
	{
		fprintf(stderr, "Invalid usage:\n\t./app <listen-port> <gw-port> <number> \"<message>\"\n");
		exit(1);
	}
	...
	str_t des = { argv[3], strlen(argv[3]) };
	str_t msg = { argv[4], strlen(argv[4]) };
	...
	LOG(L_DEBUG, "Sending [%.*s] to [%.*s]\n", msg.len, msg.s, des.len, des.s);

	dwg_send_sms(&des, &msg, port);

	char c = 0;
	do
	{
		printf("Waiting for answer or type 'q' to exit: ");
	}
	while ((c = getchar()) != 'q');
